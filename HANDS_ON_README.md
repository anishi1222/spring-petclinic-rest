...
| 時間 | セクション | エージェント | 内容 | Issue |
|------|----------|-------------|------|-------|
| 0:00-0:30 | 0️⃣ 環境セットアップ | - | アプリ起動と既存コード理解 | - |
| 0:30-1:00 | 1️⃣ 要求開発 | PM | Phase 1 Issue作成、要件定義 | `01_requirement.yml` |
| 1:00-1:30 | 2️⃣ 基本設計 | アーキテクト | Phase 2 Issue作成、システム構造・エンティティ設計 | `02_basic_design.yml` |
| 1:30-2:00 | 3️⃣ 詳細設計 | テックリード | Phase 3 Issue作成、API・DB設計 | `03_detailed_design.yml` |
| 2:00-3:30 | 4️⃣ 実装 | シニアデベロッパー | Phase 4 Issue作成、コード実装 | `04_implementation.yml` |
| 3:30-4:00 | 5️⃣ 単体テスト | QA エンジニア | Phase 5 Issue作成、テストコード作成 | `05_unit_test.yml` |
| 4:00-4:30 | 6️⃣ 統合・総合テスト | QA マネージャー | Phase 6 Issue作成、API動作確認、品質保証 | `06_integration_test.yml` |
| 4:30-5:00 | 7️⃣ 受け入れ | リード | Phase 7 Issue作成、PR作成、レビュー、マージ | `07_acceptance_review.yml` |
